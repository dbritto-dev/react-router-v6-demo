...
export default function SalesInvoicesLayout() {
  ...
  const overdueAmount = invoiceListItems.reduce(
    (output, invoice) =>
      output + (getInvoiceDueStatus(invoice) === 'overdue' ? getInvoiceTotal(invoice) : 0),
    0
  );
  const dueSoonAmount = invoiceListItems.reduce(
    (output, invoice) =>
      output + (getInvoiceDueStatus(invoice) === 'due' ? getInvoiceTotal(invoice) : 0),
    0
  );
    ...
    <div className="relative">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex h-4 flex-1 overflow-hidden rounded-full">
          ...
          <div className="bg-green-400" style={{ width: `${dueSoonPercent}%` }} />
        </div>
